#!/usr/bin/env python3
"""Write per-session average-per-slice Stage-1 timing with baseline comparison."""
...
def main():
    ...
    if set(groups) != set(baseline["group_id"].unique()):
        raise RuntimeError("candidate and baseline timing session inventories differ")

    lines = [
        "V4 STAGE1 OPT1 TIMING: SESSION AVERAGES PER SLICE",
        "",
        "Quality contract: accepted checkpoint/calibration, active_gpu, BF16, fixed batch 12,",
        ...
        "Write timings are new opt1 instrumentation and therefore show baseline n/a.",
    ...
    output.write_text("\n".join(lines))
    print(f"V4_STAGE1_OPT1_TIMING_SUMMARY_OK output={output}")
