...
def main() -> None:
    ...
    for index, row in enumerate(manifest.itertuples(index=False), 1):
        ...
        upsert_timing(timing_path, {
            ...
            "production_component_ms": float(result["timing"]["production_component_ms"]),
            "production_refiner_parametric_ms": float(result["timing"]["production_refiner_parametric_ms"]),
