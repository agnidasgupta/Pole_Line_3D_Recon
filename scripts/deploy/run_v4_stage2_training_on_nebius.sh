#!/usr/bin/env bash
POLELINE_LEGACY_SOURCE="$(cd "$(dirname "${BASH_SOURCE[0]}")/../.." && pwd)/v4/run_v4_stage2_training_on_nebius.sh"
...
ID=$("${DOCKER[@]}" run -d --name "$NAME" --gpus all \
  ...
  --mount "type=bind,source=$HERE,target=/workspace/v4,readonly" --mount "type=bind,source=$HERE/..,target=/workspace/poleline_repo,readonly" \
