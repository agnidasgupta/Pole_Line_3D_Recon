...
def pa():
 p=argparse.ArgumentParser(); p.add_argument('--stage2_dir',required=True); p.add_argument('--output_dir',required=True); p.add_argument('--session_filter',required=True); p.add_argument('--timing_csv',required=True); p.add_argument('--max_sequence_gap',type=int,default=9); p.add_argument('--max_span_length_ft',type=float,default=450.0); p.add_argument('--slice_length_ft',type=float,default=50.0); p.add_argument('--resume',type=int,default=1); p.add_argument('--span_completion_single_track_min_coverage',type=float,default=0.30); p.add_argument('--unsupported_track_min_evidence',type=float,default=2.0); p.add_argument('--unsupported_track_boundary_margin_slices',type=int,default=1); p.add_argument('--suppress_unsupported_interior_tracks',action='store_true'); return p.parse_args()
...
def main():
 ...
 for i,seq in enumerate(seqs,1):
  ...
  if a.resume and marker.is_file():resumed=True; print(f'[stage3] {i}/{len(seqs)} reuse seq={seq}',flush=True)
   ...
   dest.mkdir(parents=True,exist_ok=True); argv=['reconstruct_v4_stage3.py','--inference_dir',str(root),'--output_dir',str(dest),'--session_filter',a.session_filter,'--latest_slice',str(seq),'--max_sequence_gap',str(a.max_sequence_gap),'--max_span_length_ft',str(a.max_span_length_ft),'--slice_length_ft',str(a.slice_length_ft),'--disable_plots','--realtime_inmemory_cache','--span_completion_single_track_min_coverage',str(a.span_completion_single_track_min_coverage),'--unsupported_track_min_evidence',str(a.unsupported_track_min_evidence),'--unsupported_track_boundary_margin_slices',str(a.unsupported_track_boundary_margin_slices)];
   if a.suppress_unsupported_interior_tracks: argv.append('--suppress_unsupported_interior_tracks')
   old=sys.argv[:]; so=io.StringIO(); se=io.StringIO()
   try:
    sys.argv=argv
