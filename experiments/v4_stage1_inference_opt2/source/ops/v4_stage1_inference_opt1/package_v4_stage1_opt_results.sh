#!/usr/bin/env bash
set -euo pipefail
RUN_ROOT=$(head -n 1 /home/agni/LATEST_V4_STAGE1_OPT_RUN.txt)
[ -d "$RUN_ROOT" ] || { echo "ERROR: run root missing: $RUN_ROOT" >&2; exit 1; }
[ -s "$RUN_ROOT/STAGE1_OPT_EQUIVALENT_COMPLETE.txt" ] || { echo "ERROR: completion marker missing" >&2; exit 1; }
...
[ "$accepted" -eq 30 ] && [ "$failed" -eq 0 ] && [ "$equivalent" -eq 30 ] || {
  echo "ERROR: not ready: accepted=$accepted failed=$failed equivalent=$equivalent" >&2
  exit 1
}
stamp=$(date -u +%Y%m%dT%H%M%SZ)
archive=/home/agni/v4_stage1_opt1_equivalent_results_${stamp}.tar.gz
...
printf '%s\n' "$archive" > /home/agni/LATEST_V4_STAGE1_OPT_ARCHIVE.txt
echo "PACKAGE_OK"
