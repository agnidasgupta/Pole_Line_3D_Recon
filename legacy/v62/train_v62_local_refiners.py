...
from v62_local import LOCAL_FEATURE_COLUMNS,pole_local_physical_ok,line_local_physical_ok
...
def train(df,name,a,out):
 d=df[(df.class_name==name)&(df.target>=0)].copy(); tr=d[d.split=='train']; va=d[d.split=='val']
 ...
 w=pd.to_numeric(tr.target_weight,errors='coerce').fillna(1).to_numpy(float); clf.fit(X(tr),tr.target.to_numpy(int),sample_weight=w)
 prob=clf.predict_proba(X(va))[:,1]; physical=np.array([pole_local_physical_ok(r) if name=='pole' else line_local_physical_ok(r) for _,r in va.iterrows()])
 ...
 m.update({'threshold':best['threshold'],'average_precision':float(average_precision_score(va.target,prob)),'roc_auc':float(roc_auc_score(va.target,prob)),'train_rows':len(tr),'val_rows':len(va),'ambiguous_excluded':int(((df.class_name==name)&(df.target<0)).sum())})
 joblib.dump(clf,out/f'{name}_refiner.joblib'); search.to_csv(out/f'{name}_threshold_search.csv',index=False); plot_pr(va.target,prob,out/f'{name}_precision_recall.png',f'V6.2 Stage-2 {name} refiner')
...
def main():
 ...
 bundle={'version':'v6_2_teacher_recall_soft_stage2','feature_columns':LOCAL_FEATURE_COLUMNS,'pole_model':pm,'line_model':lm,'pole_threshold':pmet['threshold'],'line_threshold':lmet['threshold'],'pole_metrics':pmet,'line_metrics':lmet}
 joblib.dump(bundle,out/'local_refiner_bundle.joblib'); report={'completed':True,'pole':pmet,'line':lmet,'note':'World/center coordinates and pole attachments are not features. Plausible GT disagreements are excluded from training. Line acceptance uses hysteresis candidates plus a loose impossible-geometry gate so short/occluded fragments can survive to Stage 3.'}; atomic_json(report,out/'local_refiner_metrics.json'); (out/'local_refiner_metrics.txt').write_text(json.dumps(report,indent=2)); print(json.dumps(report,indent=2))
