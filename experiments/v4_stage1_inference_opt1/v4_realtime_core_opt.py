#!/usr/bin/env python3
"""Quality-equivalent Stage-1 execution optimizations.
...
serialization in the production ``v4`` package.  Only scheduling and scratch-space
management are changed here.
"""
...
OPTIMIZATION_VERSION = "v4-stage1-active-gpu-opt1-20260914"
...
def predict_v4_sparse_rows_opt(
    ...
    """Run only the previously accepted active-GPU/batch-12 inference policy."""
    if evaluate_all_cores or not gpu_coord_channels:
        raise ValueError("opt1 is restricted to accepted active_gpu mode")
    if int(batch_size) != 12:
        raise ValueError("opt1 quality contract fixes batch_size=12")
    if not fixed_batch_shape:
        raise ValueError("opt1 quality contract requires fixed_batch_shape=1")
    if amp != "bf16":
        raise ValueError("opt1 quality contract fixes amp=bf16")
