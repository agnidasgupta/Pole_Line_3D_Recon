#!/usr/bin/env python3
"""Learn Stage-1 conductor-track join geometry from Velasco without GT.

Reference ordinals 0-19 provide stable conductor thickness/separation geometry.
Target ordinals 20-39 provide the observed fragmentation-gap scale.  The output
profile is then fixed and can be applied to every session.
"""
...
from v4_stage2_stage1_track_join import (
...
def main():
    ...
    if missing: raise RuntimeError(f'manifest missing {missing}')
        ...
        if a.reference_min <= ordn <= a.reference_max:
            ...
            for i in range(len(longf)):
                for j in range(i+1,len(longf)):
                    ...
                    if _angle_deg(fa.axis_xy, fb.axis_xy) > 10.0:
                        continue
    ...
    spacing10 = pctl(ref_spacings,.10,6.0)
    max_lateral = max(.75, min(2.0, 2.5*radius95, .35*spacing10))
    provisional={
        'max_gap_ft':float(a.max_gap_cap_ft),
        'max_lateral_ft':float(max_lateral),
        'max_axis_angle_deg':15.0,
        'max_bridge_angle_deg':25.0,
        'max_vertical_gap_ft':8.0,
        'min_fragment_voxels':2,
        'vertex_bin_ft':1.0,
    }
    compatible=[]
    for ordn,seq,coords,frags in target_fragment_sets:
        for c in candidate_fragment_bridges(frags,coords,a.voxel_size_ft,provisional):
            if c['passed']:
    ...
    learned_vertical=max(3.0,min(8.0,pctl(vertical,.95,5.0)))
    profile={
        'profile_version':'velasco-stage1-track-v9-20260904',
        ...
        'max_lateral_ft':round(max_lateral,6),
        'max_axis_angle_deg':15.0,
        'max_bridge_angle_deg':25.0,
    ...
    (out/'selected_join_profile.json').write_text(json.dumps(profile,indent=2,sort_keys=True)+'\n')
    frame=pd.DataFrame(rows).sort_values('session_ordinal')
    frame.to_csv(out/'velasco_window_fragmentation.csv',index=False)
    pd.DataFrame(compatible).to_csv(out/'velasco_target_compatible_fragment_pairs.csv',index=False)
    ...
    report={
        ...
        'interpretation':(
            'V8 adjacency works when Stage1 class-2 occupancy is locally continuous. '
            'When the same conductor is split into multiple 26-neighbour components, V8 emits '
            'separate pairwise edges and cannot bridge the missing run. V9 joins compatible '
            'Stage1-backed fragments into a single conductor track.'
        ),
    }
    (out/'velasco_fragmentation_report.json').write_text(json.dumps(report,indent=2,sort_keys=True)+'\n')
    with open(out/'velasco_fragmentation_report.txt','w') as f:
        f.write('V9 STAGE1 TRACK-JOIN LEARNING\n')
        f.write(json.dumps(report,indent=2,sort_keys=True))
        f.write('\n')
    print('V9_STAGE1_TRACK_PROFILE_OK')
