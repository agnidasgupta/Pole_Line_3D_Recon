...
def main() -> None:
    ...
    coords = np.unique(np.concatenate([lower, upper], axis=0), axis=0)

    # Lower lane is a production strong seed. Upper lane is a valid deployed
    # Stage1 line label but below the production strong threshold, so it cannot
    # seed production hysteresis by itself.
    line = np.array([
        0.90 if tuple(p) in {tuple(x) for x in lower} else 0.05
        for p in coords
    ], dtype=np.float32)
    ...
    sibling = _sibling_support(upper, {"L00001": lower}, profile, voxel)
    ...
    if _sibling_support(diagonal, {"L00001": lower}, profile, voxel).get("supported"):
        raise AssertionError("crossing branch incorrectly received sibling support")

    with tempfile.TemporaryDirectory(prefix="v4-residual-union-test-") as temp:
        root = Path(temp)
        ...
        processor = Stage1ResidualUnionStage2Processor(
        ...
        expected = {tuple(map(int, p)) for p in np.concatenate([lower, upper], axis=0)}
    ...
    print("V4_STAGE2_RESIDUAL_UNION_SELF_TEST_OK")
