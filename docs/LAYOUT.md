...
On `harpreet/v4-clean-root`, obsolete root aliases have been removed as documented in [ROOT_CLEANUP.md](ROOT_CLEANUP.md). Entries marked `alias_removed` retain their canonical implementation but no longer provide the historical root path. The original migration contract below still applies to retained V4 adapters.
