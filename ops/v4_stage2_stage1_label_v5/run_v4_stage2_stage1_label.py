#!/usr/bin/env python3
"""Run V4 Stage 2 from saved Stage-1 artifacts using deployed Stage-1 labels.

This is an experiment-only Stage-2 entry point. It preserves the production
Stage-2 output contract while rebuilding line components from the exact
calibrated V4 Stage-1 inferred line mask. Ground truth is never consulted.
"""
...
from v4_stage2_stage1_label import Stage1LabelStage2Processor, profile_from_dict
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description="Stage2-only V4 experiment preserving deployed Stage1 line labels"
    )
...
def main() -> None:
    ...
    for index, row in enumerate(manifest.itertuples(index=False), 1):
        ...
        if (
            ...
            print(f"[stage2-stage1-label] {index}/{len(manifest)} reuse seq={seq}", flush=True)
        ...
        timing_row = {
            ...
            "accepted_line_components": int(audit["accepted_line_components"]),
        ...
        print(
            f"[stage2-stage1-label] {index}/{len(manifest)} seq={seq} "
    ...
    summary = {
        ...
        "stage": 2,
        "completed": True,
        "processor": "deployed_stage1_label_lane_decomposition_v5",
        "group_id": args.session_filter,
    ...
    atomic_json(summary, output_root / "STAGE2_STAGE1_LABEL_SUMMARY.json")
    print("V4_STAGE2_STAGE1_LABEL_SESSION_OK", flush=True)
