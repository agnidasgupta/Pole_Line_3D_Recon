#!/usr/bin/env python3
"""Run Stage-2-only Stage-1 inferred-line track joining from saved Stage1."""
...
from v4_stage2_stage1_track_join import STAGE1_TRACK_JOIN_RUNTIME_VERSION, Stage1TrackJoinStage2Processor
...
def diagnostic_paths(pole_csv: Path) -> dict[str, Path]:
    ...
    return {
        ...
        "audit": pole_csv.with_name(f"{stem}_stage1_track_join_audit.json"),
        ...
        "tracks": pole_csv.with_name(f"{stem}_stage1_tracks.csv"),
    }
...
def main() -> None:
    ...
    processor = Stage1TrackJoinStage2Processor(
    ...
    for index, row in enumerate(manifest.itertuples(index=False), 1):
        ...
        if args.resume and (args.session_filter, seq) in done and pole_csv.is_file() and line_csv.is_file() and vertex_csv.is_file() and ready:
            print(f"[stage2-stage1-track] {index}/{len(manifest)} reuse seq={seq}", flush=True)
        ...
        audit = {
            **result["stage1_track_join_audit"],
            ...
            "stage1_tracks_csv": str(diag["tracks"]) if args.write_voxel_audit else "",
        }
        ...
        upsert_timing(timing_path, {
            ...
            "stage1_track_join_ms": float(result["timing"]["stage1_track_join_ms"]),
        ...
        print(
            f"[stage2-stage1-track] {index}/{len(manifest)} seq={seq} "
    ...
    summary = {
        ...
        "stage": 2,
        "completed": True,
        "processor": STAGE1_TRACK_JOIN_RUNTIME_VERSION,
        "group_id": args.session_filter,
    ...
    if summary["totals"]["stage1_inferred_line_voxels"] != summary["totals"]["accepted_stage1_line_voxels"]:
        raise RuntimeError("Stage1 track-join voxel preservation invariant failed")
    atomic_json(summary, output_root / "STAGE2_COMPLETED.json")
    atomic_json(summary, output_root / "STAGE2_STAGE1_TRACK_JOIN_SUMMARY.json")
    print("V4_STAGE2_STAGE1_TRACK_JOIN_SESSION_OK", flush=True)
