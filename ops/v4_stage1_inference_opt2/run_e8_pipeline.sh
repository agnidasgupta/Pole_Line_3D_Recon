...
main() {
  ...
  if [ "$mode" = "--monitor" ]; then
    root=${2:-$(cat /home/agni/LATEST_V4_STAGE1_E8_HARNESS.txt 2>/dev/null)}
    if [ -z "$root" ]; then
  ...
  harness="$outputs/v4_stage1_e8_pipeline/$stamp"
  mkdir -p "$harness"/{roots,summaries}
  printf '%s\n' "$harness" > /home/agni/LATEST_V4_STAGE1_E8_HARNESS.txt
  ...
  launch_variant() {
    ...
    PREPARE_CORE_SCHEDULE="$pipeline" \
    PREFETCH_DEPTH=4 \
    PREFETCH_WORKERS=2 \
  ...
  if [ "$mode" = "--run" ]; then
    ...
    docker run --rm \
      ...
      "$image" python /workspace/opt/gate_e8_pipeline.py \
        ...
        --candidate "$hc/summaries/e8_pipeline_r1.json" \
        --candidate "$hc/summaries/e8_pipeline_r2.json" \
    ...
    RUN_STAMP=$(date -u +%Y%m%dT%H%M%SZ) \
    VARIANT_NAME=e8_pipeline_full30 \
    ...
    RESUME=1 \
    bash "$ops/launch_v4_stage1_opt2_experiment.sh"
    wait_for_run e8_pipeline_full30 || { printf '%s\n' "E8_STATUS=REJECTED_FULL30" > "$harness/STATE.txt"; return; }
    summarize_run e8_pipeline_full30

    full_root=$(cat "$harness/roots/e8_pipeline_full30.txt")
    printf '%s\n' "# E8 result" "" \
    ...
    cat "$harness/summaries/e8_pipeline_full30.json" >> "$harness/E8_RESULT.md"
    printf '%s\n' "~~~~" >> "$harness/E8_RESULT.md"

    archive="/home/agni/v4_stage1_e8_pipeline_${stamp}.tar.gz"
    tar -czf "$archive.partial" -C "$harness" E8_RESULT.md E8_PAIR_GATE.json summaries roots STATE.txt E8_HARNESS.log
    mv "$archive.partial" "$archive"
    sha256sum "$archive" > "$archive.sha256"
    printf '%s\n' "$archive" > /home/agni/LATEST_V4_STAGE1_E8_RESULT_ARCHIVE.txt
