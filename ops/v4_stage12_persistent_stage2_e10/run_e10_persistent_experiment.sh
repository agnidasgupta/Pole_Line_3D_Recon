#!/usr/bin/env bash
# E10 full-dataset controller: disk round-trip control vs in-memory Stage1->Stage2.
...
main() {
  ...
  group=${E10_GROUP_ID:-NYSEGDistVegMgnt_AUBURN_-_SPRUCE_HAVEN_FARMS_TAP_520-3p_2029/session2}
  stall_seconds=${E10_STALL_SECONDS:-600}
  max_attempts=${E10_MAX_ATTEMPTS:-2}
  session_timeout=${E10_SESSION_TIMEOUT_SECONDS:-7200}
  full30_control_stamp=${E10_FULL30_CONTROL_STAMP:-}

  if [ "$mode" = "--monitor" ]; then
    root=${2:-$(cat /home/agni/LATEST_V4_STAGE12_E10_HARNESS.txt 2>/dev/null)}
    echo "E10_HARNESS_ROOT=$root"
    cat "$root/STATE.txt" 2>/dev/null || true
    echo "===== RECENT HARNESS LOG ====="
    tail -n 120 "$root/E10_HARNESS.log" 2>/dev/null || true
    echo "===== DIAGNOSTICS ====="
  ...
  if [ "$mode" = "--self-test" ]; then
    if ! docker run --rm \
      ...
      --workdir /workspace/v4 -e PYTHONPATH=/workspace/v4:/workspace/e9:/workspace/e10 \
      "$image" python /workspace/e9/self_test_e9_stage12.py; then
      return 1
    fi
    if ! docker run --rm \
      --mount "type=bind,source=$e10_ops,target=/workspace/e10,readonly" \
      "$image" python -m py_compile /workspace/e10/run_e10_stage12_persistent.py /workspace/e10/summarize_e10_stage12.py; then
      echo 'E10_CODE_COMPILE_FAILED'
      return 1
    fi
    ...
    if [ -z "$stale_epoch" ] || [ $(( $(date +%s) - stale_epoch )) -lt 600 ] \
      ...
      echo 'E10_WATCHDOG_SELF_TEST_FAILED'
      return 1
    fi
    echo 'E10_WATCHDOG_SELF_TEST_OK'
    return
  fi
  # E10 fidelity controls must start from fresh roots. Reusing a prior full-30
  ...
  printf '%s\n' "$root" > /home/agni/LATEST_V4_STAGE12_E10_HARNESS.txt
  printf '%s\n' "E10_STATUS=RUNNING created_utc=$(date -u +%Y-%m-%dT%H:%M:%SZ)" > "$root/STATE.txt"
  exec > >(tee -a "$root/E10_HARNESS.log") 2>&1
  echo "E10_HARNESS_ROOT=$root"
  echo "E10_CONTRACT=exact_stage1_payload;all_stage2_outputs;serial_refiner;no_candidate_stage1_artifacts"
  echo "E10_STAGE1_CONFIG=use_cuda_graph=0;resume=0;score_atol=0;both_arms"

  package_failure() {
    if [ -s "$root/summaries/E10_RESULT.json" ]; then return; fi
    mkdir -p "$root/summaries" "$root/comparisons" "$root/diagnostics"
    printf '%s\n' '{"decision":"NOT_ACCEPTED","status":"INCOMPLETE","reason":"The full validation did not complete. Review STATE.txt, E10_HARNESS.log and diagnostics before a controlled restart."}' > "$root/summaries/E10_RESULT.json"
    {
      echo '# E10 in-memory handoff: incomplete run'
      echo
    ...
    } > "$root/summaries/E10_RESULT.md"
    failure_archive="/home/agni/v4_stage12_e10_${stamp}_incomplete.tar.gz"
    tar -czf "$failure_archive.partial" -C "$root" STATE.txt E10_HARNESS.log summaries comparisons diagnostics 2>/dev/null && {
      mv "$failure_archive.partial" "$failure_archive"
      sha256sum "$failure_archive" > "$failure_archive.sha256"
      printf '%s\n' "$failure_archive" > /home/agni/LATEST_V4_STAGE12_E10_RESULT_ARCHIVE.txt
      echo "E10_DIAGNOSTIC_ARCHIVE=$failure_archive"
    }
  }
  ...
  if ! docker run --rm \
    ...
    --workdir /workspace/v4 -e PYTHONPATH=/workspace/v4:/workspace/e9:/workspace/e10 \
    "$image" python /workspace/e9/self_test_e9_stage12.py; then
    printf '%s\n' 'E10_STATUS=STOP_SELF_TEST_FAILED' > "$root/STATE.txt"
    return
  fi
  if ! docker run --rm \
    --mount "type=bind,source=$e10_ops,target=/workspace/e10,readonly" \
    "$image" python -m py_compile /workspace/e10/run_e10_stage12_persistent.py /workspace/e10/summarize_e10_stage12.py; then
    printf '%s\n' 'E10_STATUS=STOP_CODE_COMPILE_FAILED' > "$root/STATE.txt"
    return
  fi
  ...
  run_watched() {
    ...
    while kill -0 "$watched_pid" 2>/dev/null; do
      ...
      if [ $((now-last_change)) -gt "$stall_seconds" ]; then
        echo "E10_STALL label=$label pid=$watched_pid"
  ...
  wait_stage1_driver() {
    ...
    while [ -n "$pid" ] && kill -0 "$pid" 2>/dev/null; do
      ...
      if [ $((now-last_change)) -gt "$stall_seconds" ]; then
        echo "E10_STAGE1_CONTROL_STALL label=$label pid=$pid last_activity_epoch=$last_change idle_seconds=$((now-last_change))"
        capture_diagnostics "$label-stall" "$log" "$run"
        kill -TERM "$pid" 2>/dev/null || true
        break
      fi
      printf '%s\n' "E10_STATUS=RUNNING_STAGE1_CONTROL label=$label pid=$pid" > "$root/STATE.txt"
      sleep 30
    done
  ...
  run_disk_stage2() {
    ...
    for manifest in "${manifests[@]}"; do
      ...
      run_watched "$label-stage2-$sid" "$log" timeout --signal=TERM --kill-after=120 "$session_timeout" docker run --rm \
        ...
        --workdir /workspace/v4 -e PYTHONPATH=/workspace/v4:/workspace/stage1opt:/workspace/stage2:/workspace/e9:/workspace/e10 \
  ...
  run_candidate_session() {
    ...
    while [ "$attempt" -le "$max_attempts" ]; do
      ...
      timeout --signal=TERM --kill-after=120 "$session_timeout" docker run --rm --gpus all \
        ...
        --workdir /workspace/v4 -e PYTHONPATH=/workspace/v4:/workspace/stage1opt:/workspace/stage2:/workspace/e9:/workspace/e10 \
        "$image" python /workspace/e10/run_e10_stage12_persistent.py \
          ...
          --input_dir /data/voxel_csv_combined --session_filter "$gid" --output_dir "/tmp/e10-stage1-$sid" \
          --model_path "$mc" --calibration_json "$cc" --timing_csv "/tmp/e10-stage1-$sid.csv" --progress_json "$progressc" \
      ...
      while kill -0 "$worker" 2>/dev/null; do
        ...
        if [ $((now-last_change)) -gt "$stall_seconds" ]; then
          echo "E10_CANDIDATE_STALL sid=$sid attempt=$attempt worker=$worker"
  ...
  run_candidate_all() {
    ...
    if [ "${#manifests[@]}" -ne "$expected" ]; then echo "E10_STATUS=STOP_SESSION_COUNT control=${#manifests[@]} expected=$expected"; return 1; fi
    count=0
    for manifest in "${manifests[@]}"; do
      gid=$(gid_from_manifest "$manifest"); sid=$(basename "$(dirname "$manifest")")
      ...
      count=$((count+1)); printf '%s\n' "E10_STATUS=RUNNING_CANDIDATE label=$label sessions=$count/$expected" > "$root/STATE.txt"
    done
    return 0
  }
  # Two repeated representative checks catch nondeterminism before all-session work.
  for repeat in 1 2; do
    c_label="e10_control_r$repeat"; i_label="e10_persistent_r$repeat"
    control_s1=$(launch_control_stage1 "$c_label" 1 "$group") || { printf '%s\n' "E10_STATUS=REJECTED_${c_label}" > "$root/STATE.txt"; return; }
    run_disk_stage2 "$c_label" "$control_s1" "$root/control/$c_label/stage2" || { printf '%s\n' "E10_STATUS=REJECTED_STAGE2_${c_label}" > "$root/STATE.txt"; return; }
    run_candidate_all "$i_label" "$c_label" "$control_s1" "$root/candidate/$i_label" 1 || { printf '%s\n' "E10_STATUS=REJECTED_${i_label}" > "$root/STATE.txt"; return; }
  done
  control_s1=$(launch_control_stage1 e10_control_full30 30 "") || { printf '%s\n' "E10_STATUS=REJECTED_CONTROL_FULL30" > "$root/STATE.txt"; return; }
  run_disk_stage2 e10_control_full30 "$control_s1" "$root/control/e10_control_full30/stage2" || { printf '%s\n' "E10_STATUS=REJECTED_STAGE2_CONTROL_FULL30" > "$root/STATE.txt"; return; }
  run_candidate_all e10_persistent_full30 e10_control_full30 "$control_s1" "$root/candidate/e10_persistent_full30" 30 || { printf '%s\n' "E10_STATUS=REJECTED_PERSISTENT_FULL30" > "$root/STATE.txt"; return; }
  controlc=$(host_to_container "$control_s1/timings/stage1"); stage2c=$(host_to_container "$root/control/e10_control_full30/stage2/timings"); candidatec=$(host_to_container "$root/candidate/e10_persistent_full30/timings/stage12"); comparisonsc=$(host_to_container "$root/comparisons"); jsonc=$(host_to_container "$root/summaries/E10_RESULT.json"); mdc=$(host_to_container "$root/summaries/E10_RESULT.md")
  if ! docker run --rm --mount "type=bind,source=$repo/v4,target=/workspace/v4,readonly" --mount "type=bind,source=$e10_ops,target=/workspace/e10,readonly" --mount "type=bind,source=$outputs,target=/outputs" --workdir /workspace/v4 -e PYTHONPATH=/workspace/v4:/workspace/e10 "$image" \
    python /workspace/e10/summarize_e10_stage12.py --control-stage1-timings "$controlc" --control-stage2-timings "$stage2c" --candidate-timings "$candidatec" --comparison-dir "$comparisonsc" --json "$jsonc" --markdown "$mdc"; then
    printf '%s\n' 'E10_STATUS=STOP_SUMMARY_FAILED' > "$root/STATE.txt"
    return
  fi
  decision=$(sed -n 's/^[[:space:]]*"decision": "\([^"]*\)".*/\1/p' "$root/summaries/E10_RESULT.json")
  if [ "$decision" != ACCEPTED ] && [ "$decision" != NOT_ACCEPTED ]; then
    echo 'E10_STATUS=STOP_INVALID_DECISION' > "$root/STATE.txt"
    return
  fi
  printf '%s\n' "E10_STATUS=$decision" > "$root/STATE.txt"
  ...
  cp "$control_s1/timings/stage1/"*.csv "$root/summaries/control_stage1_raw_timings/" || {
    echo "E10_STATUS=STOP_CONTROL_RAW_TIMINGS_MISSING" > "$root/STATE.txt"
    return
  }
  archive="/home/agni/v4_stage12_e10_${stamp}.tar.gz"
  if ! tar -czf "$archive.partial" -C "$root" STATE.txt E10_HARNESS.log summaries comparisons diagnostics candidate/e10_persistent_full30/timings candidate/e10_persistent_full30/status control/e10_control_full30/stage2/timings; then
    echo "E10_STATUS=STOP_RESULT_PACKAGE_FAILED" > "$root/STATE.txt"
    echo "E10_RESULT_PACKAGE_FAILED partial=$archive.partial"
    return
  fi
  mv "$archive.partial" "$archive"
  sha256sum "$archive" > "$archive.sha256"
  printf '%s\n' "$archive" > /home/agni/LATEST_V4_STAGE12_E10_RESULT_ARCHIVE.txt
  echo "E10_RESULT_ARCHIVE=$archive"
  echo "E10_DECISION=$decision"
}
