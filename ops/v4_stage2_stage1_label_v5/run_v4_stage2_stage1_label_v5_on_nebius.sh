...
# Stage2-only experiment. The output contract is intentionally identical to the
...
TOOL_DIR="$EXP_REPO/ops/v4_stage2_stage1_label_v5"
...
for file in \
  v4_stage2_stage1_label.py \
  select_v4_stage2_stage1_label_profile.py \
  run_v4_stage2_stage1_label.py \
  self_test_stage1_label.py; do
...
docker run --rm -i \
  ...
  bash -lc "
    ...
    python -m py_compile \
      /workspace/quality/v4_stage2_stage1_label.py \
      /workspace/quality/select_v4_stage2_stage1_label_profile.py \
      /workspace/quality/run_v4_stage2_stage1_label.py \
      /workspace/quality/self_test_stage1_label.py
    python /workspace/quality/self_test_stage1_label.py
    python /workspace/quality/select_v4_stage2_stage1_label_profile.py --self_test
...
if [ "$PREFLIGHT_ONLY" = "1" ]; then
  echo "============================================================"
  echo "V4 STAGE2 STAGE1-LABEL V5 PREFLIGHT OK"
  echo "============================================================"
...
printf '%s\n' "$RUN_HOST" > "$HOME/LATEST_V4_STAGE2_STAGE1_LABEL_V5_RUN.txt"
...
experiment=deployed_stage1_label_preservation_and_parallel_lane_decomposition_v5
repository=$EXP_REPO
...
runtime_line_source=deployed_v4_label_from_scores_class_2
...
docker run --rm \
  ...
  "$IMAGE" \
  python /workspace/quality/select_v4_stage2_stage1_label_profile.py \
...
if [ "$SELECT_EXIT" -ne 0 ]; then
  if [ "$SELECT_EXIT" -eq 3 ] && [ -f "$SELECTION_HOST/NO_SAFE_STAGE1_LABEL_PROFILE.txt" ]; then
    cp "$SELECTION_HOST/NO_SAFE_STAGE1_LABEL_PROFILE.txt" "$RUN_HOST/NO_SAFE_STAGE1_LABEL_PROFILE.txt"
    ...
    ARCHIVE="$PACKAGE_HOST/v4_stage2_stage1_label_selection_full_run_v2_${RUN_STAMP}.tar.gz"
    tar -czf "$ARCHIVE" -C "$RUN_HOST" selection logs RUN_INFO.txt NO_SAFE_STAGE1_LABEL_PROFILE.txt FILE_INVENTORY.txt
    tar -tzf "$ARCHIVE" >/dev/null
    sha256sum "$ARCHIVE" > "${ARCHIVE}.sha256"
    printf '%s\n' "$ARCHIVE" > "$HOME/LATEST_V4_STAGE2_STAGE1_LABEL_V5_ARCHIVE.txt"
    echo "NO_SAFE_STAGE1_LABEL_PROFILE"
...
grep -Fq 'SELECTED_SAFE_STAGE1_LABEL_PROFILE' "$SELECTION_HOST/selection_report.txt" \
...
for manifest in "${MANIFESTS[@]}"; do
  ...
  echo "============================================================"
  echo "STAGE2 STAGE1-LABEL $INDEX/$TOTAL"
  echo "GID=$GID"
  echo "SID=$SID"
  ...
  docker run --rm \
    ...
    -e MPLCONFIGDIR=/tmp/matplotlib \
    "$IMAGE" \
    python /workspace/quality/run_v4_stage2_stage1_label.py \
  ...
  [ -f "$S2_SESSION_HOST/STAGE2_STAGE1_LABEL_SUMMARY.json" ] || fail "Stage1-label summary missing for $GID"
...
ARCHIVE="$PACKAGE_HOST/v4_stage2_stage1_label_${RUN_ID}.tar.gz"
...
echo "============================================================"
echo "V4 STAGE2 STAGE1-LABEL CANONICAL RUN COMPLETE"
echo "============================================================"
