...
NAME=${NAME:-poleline-v4-realtime-session}
OUT=${OUT:-/outputs/poleline_voxel_run_session_groups/v4_realtime/replays/$(echo "$SESSION_FILTER" | tr '/ ' '__')}
...
sudo docker run -d --name "$NAME" --gpus all \
  ...
  python run_v4_realtime_session.py \
    --input_dir "${INPUT_DIR:-/data/voxel_csv_combined}" \
    --session_filter "$SESSION_FILTER" --output_dir "$OUT" \
    --model_path "${MODEL:-/outputs/poleline_voxel_run_session_groups/precision_v4/train/precision_best.pt}" \
    --calibration_json "${CAL:-/outputs/poleline_voxel_run_session_groups/precision_v4/full_val/calibration.json}" \
    --stage2_bundle "${STAGE2_BUNDLE:-/outputs/poleline_voxel_run_session_groups/v4_realtime/stage2_refiner/local_refiner_bundle.joblib}" \
    --batch_size "${BATCH_SIZE:-12}" --amp "${AMP:-bf16}" --compile_model "${COMPILE_MODEL:-0}" --evaluate_all_cores "${EVALUATE_ALL_CORES:-1}" --gpu_coord_channels "${GPU_COORD_CHANNELS:-0}" \
    --max_span_slices 9 --max_span_length_ft 450 \
    --stage3_every_slice 1 --stage3_execution "${STAGE3_EXECUTION:-inprocess}" --finalize_full_session 0 \
