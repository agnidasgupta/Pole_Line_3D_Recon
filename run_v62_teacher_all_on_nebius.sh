...
sudo docker run -d --name "$NAME" --gpus all --shm-size=64g \
  ...
  -e TRAIN_BATCH="${TRAIN_BATCH:-3}" -e GRAD_ACCUM="${GRAD_ACCUM:-2}" -e EVAL_BATCH="${EVAL_BATCH:-5}" -e INFER_BATCH="${INFER_BATCH:-5}" \
  ...
  --workdir /workspace/voxel_poleline "$IMAGE" bash -lc './run_v62_teacher_all.sh'
