#!/usr/bin/env python3
"""V6.2 Stage 3 clean-track reconstruction.
...
def pole_attachment_candidate(endpoint, outward_tangent, poles, a, exclude=None):
    """Return a geometrically supported pole attachment without stretching the pole unrealistically."""
    if poles.empty:
        return None
    exclude = set(exclude or [])
    best = None
    for idx, r in poles.iterrows():
        if idx in exclude:
            continue
...
def main():
    ...
    for _, r in man.iterrows():
        ...
        if not l.empty and not v.empty:
            for _, q in l.iterrows():
                z = v[v.component_id.astype(str) == str(q.component_id)].sort_values("vertex_index")
    ...
    for gid in group_ids:
        ...
        print(f"[stage3-phase] {gid} fragment_join_seconds={time.time()-t_join:.1f} accepted={len(accepted)}", flush=True)
        ...
        for track_idx, tr in enumerate(tracks):
            ...
            observed_start = pole_attachment_candidate(raw[0], start_t, mp, a)
            observed_end = pole_attachment_candidate(raw[-1], end_t, mp, a)
        ...
        print(f"[stage3-phase] {gid} span_completion_pre_seconds={time.time()-t_span:.1f} paths={len(span_completion_audit_pre)}", flush=True)
        ...
        print(f"[stage3-phase] {gid} hidden_pole_seconds={time.time()-t_hidden:.1f} inferred={len(hidden_mp)}", flush=True)
        ...
        print(f"[stage3-phase] {gid} span_completion_post_seconds={time.time()-t_span:.1f} paths={len(span_completion_audit_post)}", flush=True)
        ...
        for tr in track_records:
            ...
            if cb is not None and int(cb.get("idx", -1)) not in mp.index:
                cb = None
            if ca is None:
                ca = pole_attachment_candidate(raw[0], start_t, mp, a, exclude=[cb["idx"]] if cb else None)
            if cb is None:
                cb = pole_attachment_candidate(raw[-1], end_t, mp, a, exclude=[ca["idx"]] if ca else None)

            # If the first choice prevents a valid second distinct pole, also try the opposite order.
            if ca is None and cb is not None:
                ca = pole_attachment_candidate(raw[0], start_t, mp, a, exclude=[cb["idx"]])
            if cb is None and ca is not None:
                cb = pole_attachment_candidate(raw[-1], end_t, mp, a, exclude=[ca["idx"]])
        ...
        print("[stage3-circuit-complete]", gid, summary, f"elapsed_seconds={time.time()-session_started:.1f}", flush=True)
    ...
    atomic_json({
        "completed": True,
        "reconstruction_version": "v6.2-circuit-complete",
