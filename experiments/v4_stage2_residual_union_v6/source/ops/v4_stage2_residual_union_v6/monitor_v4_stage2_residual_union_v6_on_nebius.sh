...
RUN_POINTER="${RUN_POINTER:-$HOME/LATEST_V4_STAGE2_RESIDUAL_UNION_V6_RUN.txt}"
PID_POINTER="${PID_POINTER:-$HOME/LATEST_V4_STAGE2_RESIDUAL_UNION_V6_PID.txt}"
MASTER_LOG="${MASTER_LOG:-$HOME/v4_stage2_residual_union_v6_master.log}"
...
elif [ -f "$RUN_HOST/NO_SAFE_RESIDUAL_UNION_PROFILE.txt" ]; then
  SELECTION="NO_SAFE_RESIDUAL_UNION_PROFILE"
fi
...
cat <<EOF
============================================================
V4 STAGE2 RESIDUAL-UNION V6 STATUS
============================================================
Run:          $RUN_HOST
Process:      $PROCESS
Selection:    $SELECTION
...
if [ -f "$RUN_HOST/STAGE2_ONLY_COMPLETE.txt" ]; then
  ...
  if [ -f "$HOME/LATEST_V4_STAGE2_RESIDUAL_UNION_V6_ARCHIVE.txt" ]; then
    ARCHIVE=$(cat "$HOME/LATEST_V4_STAGE2_RESIDUAL_UNION_V6_ARCHIVE.txt")
    echo "Archive: $ARCHIVE"
    ls -lh "$ARCHIVE" "${ARCHIVE}.sha256"
  fi
elif [ "$PROCESS" != "RUNNING"* ] && [ "$SELECTION" != "NO_SAFE_RESIDUAL_UNION_PROFILE" ]; then
  echo
