...
def _pole_attachment_candidate(
    ...
    pole_support_index: dict[tuple[int, int, int], list[tuple[int, np.ndarray]]],
    poles: pd.DataFrame | None,
    voxel_size_ft: float,
    profile: dict[str, Any],
) -> dict[str, Any] | None:
    if poles is None or poles.empty or not pole_support_index:
        return None
    ...
    radius = max(1, int(math.ceil(contact_max)))
    px, py, pz = _voxel_key(p)
    indexed_contacts: list[tuple[int, np.ndarray]] = []
    for dx in range(-radius, radius + 1):
        for dy in range(-radius, radius + 1):
            for dz in range(-radius, radius + 1):
                indexed_contacts.extend(pole_support_index.get((px + dx, py + dy, pz + dz), ()))
    indexed_contacts.sort(key=lambda pair: pair[0])
    contacts = [q for _, q in indexed_contacts
                if np.max(np.abs(q - p)) <= contact_max + 1.0e-12
                and np.linalg.norm(q - p) > 1.0e-12]
    if not len(contacts):
        return None
    best = None
    for r in poles.itertuples(index=False):
...
def _attach_track_to_poles(
    ...
    for end, endpoint in (("start", v[0]), ("end", v[-1])):
        candidate = _pole_attachment_candidate(
            endpoint, _track_endpoint_direction(v, end), pole_support_index,
...
def build_electrical_track_outputs(
    ...
    line_connected_components_ms = (time.perf_counter() - phase_t0) * 1000.0
    pole_support_voxels = c[lab == 1].copy()
    pole_support_index: dict[tuple[int, int, int], list[tuple[int, np.ndarray]]] = {}
    for pole_order, pole_voxel in enumerate(pole_support_voxels):
        pole_support_index.setdefault(_voxel_key(pole_voxel), []).append(
            (int(pole_order), np.asarray(pole_voxel, dtype=float))
        )

    # The accepted strict model forbids every disconnected-component bridge.
    # The previous diagnostic enumerated all F*(F-1)/2 pairs only to reject
    # them.  Avoiding that dead path changes no reconstructed output.
    candidates: list[dict[str, Any]] = []
    ...
    line_trace_ms = (time.perf_counter() - phase_t0) * 1000.0
    ...
    line_assignment_audit_ms = (time.perf_counter() - phase_t0) * 1000.0
    ...
    for track_no, trace in enumerate(traces, 1):
        ...
        verts, rows = _attach_track_to_poles(
            verts, pole_support_index, poles, voxel_size_ft, profile,
    ...
    line_geometry_ms = (time.perf_counter() - phase_t0) * 1000.0
    blocked_support = 0
    return {
        ...
        "raw_components": components, "fragments": [],
        ...
        "timing": {
            "line_connected_components_ms": float(line_connected_components_ms),
            "line_trace_ms": float(line_trace_ms),
            "line_assignment_audit_ms": float(line_assignment_audit_ms),
            "line_geometry_ms": float(line_geometry_ms),
            "disconnected_bridge_pair_enumeration_ms": 0.0,
        },
    }
        ...
        return {
            ...
            "timing": {
                ...
                **joined["timing"],
                ...
                "stage2_total_ms": float(baseline_ms + label_resolve_ms + join_ms),
            },
        }
