...
REMOTE_POINTER="${REMOTE_POINTER:-\$HOME/LATEST_V4_STAGE2_RESIDUAL_UNION_V6_ARCHIVE.txt}"
...
case "$ARCHIVE_NAME" in
  v4_stage2_residual_union_full_run_v2_*.tar.gz)
    RUN_ID=${ARCHIVE_NAME#v4_stage2_residual_union_}
    RUN_ID=${RUN_ID%.tar.gz}
    ;;
  v4_stage2_residual_union_selection_full_run_v2_*.tar.gz)
    RUN_ID=${ARCHIVE_NAME#v4_stage2_residual_union_selection_}
    RUN_ID=${RUN_ID%.tar.gz}
...
if [ -f "$LOCAL_RUN/STAGE2_ONLY_COMPLETE.txt" ]; then
  ...
  echo "V4_STAGE2_RESIDUAL_UNION_DOWNLOAD_OK"
else
  [ -f "$LOCAL_RUN/NO_SAFE_RESIDUAL_UNION_PROFILE.txt" ] || {
    echo "ERROR: neither success nor no-safe-profile marker found" >&2
    exit 1
  }
  echo "V4_STAGE2_RESIDUAL_UNION_DIAGNOSTIC_DOWNLOAD_OK"
fi
