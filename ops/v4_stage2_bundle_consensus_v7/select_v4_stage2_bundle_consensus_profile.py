#!/usr/bin/env python3
"""Select a production-preserving Stage1 bundle-consensus V4 Stage2 profile with offline GT.
...
def generated_profiles() -> list[Stage1LabelProfile]:
    variants = [
        Stage1LabelProfile(
            ...
            residual_min_novel_voxels=8,
            residual_min_novel_fraction=0.75,
            residual_max_baseline_overlap_fraction=0.25,
            sibling_max_axis_angle_deg=5.0,
            sibling_min_cross_section_offset_ft=0.75,
            sibling_max_cross_section_offset_ft=6.0,
            sibling_min_longitudinal_overlap_fraction=0.70,
            bundle_min_parallel_siblings=2,
            bundle_min_endpoint_overlap_fraction=0.88,
            bundle_max_endpoint_extension_ft=1.5,
            bundle_spacing_ratio_min=0.65,
            bundle_spacing_ratio_max=1.40,
        ),
        Stage1LabelProfile(
            name="strict_e055_m2",
            ...
            residual_min_novel_voxels=6,
            residual_min_novel_fraction=0.60,
            residual_max_baseline_overlap_fraction=0.35,
            sibling_max_axis_angle_deg=6.0,
            sibling_min_cross_section_offset_ft=0.50,
            sibling_max_cross_section_offset_ft=8.0,
            sibling_min_longitudinal_overlap_fraction=0.60,
            bundle_min_parallel_siblings=2,
            bundle_min_endpoint_overlap_fraction=0.84,
            bundle_max_endpoint_extension_ft=2.0,
            bundle_spacing_ratio_min=0.60,
            bundle_spacing_ratio_max=1.50,
        ),
        Stage1LabelProfile(
            ...
            residual_min_novel_fraction=0.50,
            residual_max_baseline_overlap_fraction=0.45,
            sibling_max_axis_angle_deg=8.0,
            sibling_min_cross_section_offset_ft=0.50,
            sibling_max_cross_section_offset_ft=10.0,
            sibling_min_longitudinal_overlap_fraction=0.50,
            bundle_min_parallel_siblings=2,
            bundle_min_endpoint_overlap_fraction=0.80,
            bundle_max_endpoint_extension_ft=2.5,
            bundle_spacing_ratio_min=0.55,
            bundle_spacing_ratio_max=1.60,
        ),
        Stage1LabelProfile(
            ...
            residual_min_novel_voxels=4,
            residual_min_novel_fraction=0.40,
            residual_max_baseline_overlap_fraction=0.50,
            sibling_max_axis_angle_deg=9.0,
            sibling_min_cross_section_offset_ft=0.50,
            sibling_max_cross_section_offset_ft=12.0,
            sibling_min_longitudinal_overlap_fraction=0.40,
            bundle_min_parallel_siblings=3,
            bundle_min_endpoint_overlap_fraction=0.72,
            bundle_max_endpoint_extension_ft=3.0,
            bundle_spacing_ratio_min=0.45,
            bundle_spacing_ratio_max=1.75,
        ),
        Stage1LabelProfile(
            name="recall_e085_m2",
            ...
            residual_min_novel_voxels=3,
            residual_min_novel_fraction=0.30,
            residual_max_baseline_overlap_fraction=0.60,
            sibling_max_axis_angle_deg=10.0,
            sibling_min_cross_section_offset_ft=0.25,
            sibling_max_cross_section_offset_ft=14.0,
            sibling_min_longitudinal_overlap_fraction=0.35,
            bundle_min_parallel_siblings=3,
            bundle_min_endpoint_overlap_fraction=0.65,
            bundle_max_endpoint_extension_ft=4.0,
            bundle_spacing_ratio_min=0.40,
            bundle_spacing_ratio_max=1.90,
        ),
        replace(
            Stage1LabelProfile(),
            name="no_override_consensus2",
            enable_geometry_override=False,
            bundle_min_parallel_siblings=2,
            bundle_min_endpoint_overlap_fraction=0.82,
            bundle_max_endpoint_extension_ft=2.0,
            bundle_spacing_ratio_min=0.60,
            bundle_spacing_ratio_max=1.50,
            sibling_min_longitudinal_overlap_fraction=0.65,
            residual_min_novel_voxels=6,
            residual_min_novel_fraction=0.60,
        ),
    ]
...
def write_profile_env(path: Path, profile: Stage1LabelProfile) -> None:
    with path.open("w") as handle:
        handle.write("SELECTION_STATUS=SELECTED_SAFE_BUNDLE_CONSENSUS_PROFILE\n")
...
def self_test() -> None:
    ...
    print(f"V4_STAGE2_BUNDLE_CONSENSUS_SELECTOR_SELF_TEST_OK version={BUNDLE_CONSENSUS_SELECTOR_VERSION}")


def main() -> None:
    args = cli()
    print(f"BUNDLE_CONSENSUS_SELECTOR_VERSION={BUNDLE_CONSENSUS_SELECTOR_VERSION}", flush=True)
    if args.self_test:
        self_test()
        return
    ...
    (out / "SELECTOR_VERSION.txt").write_text(BUNDLE_CONSENSUS_SELECTOR_VERSION + "\n")
    ...
    if selected is None:
        (out / "NO_SAFE_BUNDLE_CONSENSUS_PROFILE.txt").write_text(
            "NO_SAFE_BUNDLE_CONSENSUS_PROFILE\n"
        ...
        print("NO_SAFE_BUNDLE_CONSENSUS_PROFILE")
        raise SystemExit(3)
    ...
    selected_processor = Stage1BundleConsensusStage2Processor(
    ...
    report = {
        "status": "SELECTED_SAFE_BUNDLE_CONSENSUS_PROFILE",
        ...
        "architecture": "production_stage2_union_stage1_label_residual_parallel_sibling",
    ...
    with (out / "selection_report.txt").open("w") as handle:
        handle.write("SELECTED_SAFE_BUNDLE_CONSENSUS_PROFILE\n")
        ...
        handle.write("architecture=production_stage2_union_stage1_label_residual_parallel_sibling\n")
        handle.write("runtime_gt_usage=false\n")
        handle.write("synthetic_line_voxels=0\n")
    print("SELECTED_SAFE_BUNDLE_CONSENSUS_PROFILE")
