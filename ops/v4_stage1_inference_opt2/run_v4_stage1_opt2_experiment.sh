...
if [ "$CACHE_REFERENCE_COORDINATE_CHANNELS" = 1 ] && { [ "$CACHE_COORDINATE_CHANNELS" != 0 ] || [ "$PRECOMPUTE_BATCH_GATHER_PLANS" != 0 ] || [ "$RETAIN_GATHER_HOST_BUFFERS" != 0 ]; }; then
  fail "E5b isolates reference coordinate caching and requires E5, E4, and E3 to be disabled"
fi
