...
from v4_stage2_stage1_electrical_tracks_opt import STAGE1_ELECTRICAL_TRACK_RUNTIME_VERSION, Stage1ElectricalTrackStage2Processor
...
def main() -> None:
    ...
    for index, row in enumerate(manifest.itertuples(index=False), 1):
        slice_wall_t0 = time.perf_counter()
        ...
        output_write_ms = (time.perf_counter() - output_write_t0) * 1000.0
        slice_wall_ms = (time.perf_counter() - slice_wall_t0) * 1000.0
        ...
        upsert_timing(timing_path, {
            ...
            "production_pole_component_ms": float(result["timing"]["production_pole_component_ms"]),
            "production_pole_refiner_parametric_ms": float(result["timing"]["production_pole_refiner_parametric_ms"]),
            "stage1_label_resolve_ms": float(result["timing"]["stage1_label_resolve_ms"]),
            "line_connected_components_ms": float(result["timing"]["line_connected_components_ms"]),
            "line_trace_ms": float(result["timing"]["line_trace_ms"]),
            "line_assignment_audit_ms": float(result["timing"]["line_assignment_audit_ms"]),
            "line_geometry_ms": float(result["timing"]["line_geometry_ms"]),
            "disconnected_bridge_pair_enumeration_ms": float(result["timing"]["disconnected_bridge_pair_enumeration_ms"]),
            "stage1_electrical_track_ms": float(result["timing"]["stage1_electrical_track_ms"]),
            "stage2_total_ms": stage2_ms,
            "stage2_output_write_ms": float(output_write_ms),
            "slice_wall_ms": float(slice_wall_ms),
