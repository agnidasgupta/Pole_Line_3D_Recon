...
# Canonical target/all Stage-2 runner for V10 electrical-safe Stage-1 conductor tracks.
...
is_nonnegative_integer "$EXPECTED_SESSIONS" || fail "EXPECTED_SESSIONS must be a nonnegative integer"

[ -d "$EXP_REPO/.git" ] || [ -f "$EXP_REPO/.git" ] || fail "experiment Git worktree missing: $EXP_REPO"
[ -d "$TOOL_DIR" ] || fail "V10 tool directory missing: $TOOL_DIR"
for f in run_v4_stage2_stage1_electrical_tracks.py v4_stage2_stage1_electrical_tracks.py learn_velasco_stage1_electrical_profile.py self_test_stage1_electrical_tracks.py; do
  [ -f "$TOOL_DIR/$f" ] || fail "V10 required file missing: $TOOL_DIR/$f"
done
...
if ! git -C "$EXP_REPO" diff --quiet "$PRODUCTION_TAG" -- v4; then
  fail "production V4 implementation differs from $PRODUCTION_TAG; V10 code must remain under ops only"
fi
...
if [ "$EXPECTED_SESSIONS" -gt 0 ] && [ "${#ALL_MANIFESTS[@]}" -ne "$EXPECTED_SESSIONS" ]; then
  fail "Stage1 session count mismatch: expected=$EXPECTED_SESSIONS found=${#ALL_MANIFESTS[@]}"
fi
...
docker run --rm \
  ...
  bash -lc '
    ...
    python -c "from v4_realtime_pipeline import V4Stage2Processor; import v4_stage2_stage1_electrical_tracks as t; assert V4Stage2Processor.__module__ == \"v4_realtime_pipeline\"; print(\"V10_ELECTRICAL_TRACK_IMPORT_OK\"); print(t.STAGE1_ELECTRICAL_TRACK_RUNTIME_VERSION)"
    rm -rf /tmp/v10profile /tmp/v10stage2 /tmp/v10time.csv
    python /workspace/quality/learn_velasco_stage1_electrical_profile.py \
      ...
      --output_dir /tmp/v10profile
    python /workspace/quality/run_v4_stage2_stage1_electrical_tracks.py \
      --stage1_dir "'$STAGE1_ROOT_C'/VELASCO_CUT_CP__session1" \
      --output_dir /tmp/v10stage2 \
      --session_filter "'$TARGET_GID'" \
      --stage2_bundle "'$STAGE2_BUNDLE_C'" \
      --calibration_json "'$CALIBRATION_C'" \
      --profile_json /tmp/v10profile/selected_electrical_profile.json \
      --timing_csv /tmp/v10time.csv \
      --resume 0 --max_slices 1 --write_voxel_audit 1
    test -f /tmp/v10stage2/STAGE2_STAGE1_ELECTRICAL_TRACK_SUMMARY.json
    echo V10_STAGE1_ELECTRICAL_REAL_SLICE_PREFLIGHT_OK
  '


if [ "$PREFLIGHT_ONLY" = "1" ]; then
  echo "============================================================"
  echo "V10 STAGE1 ELECTRICAL CANONICAL PREFLIGHT OK"
  echo "============================================================"
  echo "repository=$EXP_REPO"
  echo "commit=$COMMIT"
  echo "stage1_root=$STAGE1_ROOT_HOST"
  echo "stage1_sessions=${#ALL_MANIFESTS[@]}"
  echo "expected_sessions=$EXPECTED_SESSIONS"
...
printf '%s\n' "$RUN_HOST" > "$HOME/LATEST_V4_STAGE2_STAGE1_ELECTRICAL_V10_RUN.txt"
...
PROFILE_HOST="$SELECTION_HOST/selected_electrical_profile.json"
...
experiment=stage1_inferred_electrical_tracks_v10
repository=$EXP_REPO
...
runtime_line_source=deployed_v4_stage1_class_2_electrical_tracks
...
pole_endpoint_attachment_enabled=true
fragment_bridge_source=stage1_class2_same_lane_end_to_end_only
electrical_profile=$PROFILE_HOST
...
for manifest in "${MANIFESTS[@]}"; do
  ...
  echo "============================================================"
  echo "STAGE2 STAGE1-ELECTRICAL $INDEX/$TOTAL"
  echo "GID=$GID"
  echo "SID=$SID"
  ...
  set -e
  [ "$SESSION_RC" -eq 0 ] || fail "Stage2 electrical-track run failed for $GID rc=$SESSION_RC"

  [ -f "$S2_SESSION_HOST/STAGE2_COMPLETED.json" ] || fail "Stage2 completion marker missing for $GID"
  [ -f "$S2_SESSION_HOST/STAGE2_STAGE1_ELECTRICAL_TRACK_SUMMARY.json" ] || fail "Stage1 electrical-track summary missing for $GID"
  ...
  # Verify every emitted per-slice audit preserves Stage1 class-2 voxels exactly and enforce electrical invariants.
  docker run --rm -i \
...
files = sorted(glob.glob('/review/**/*_stage1_electrical_track_audit.json', recursive=True))
if not files:
    raise SystemExit('no *_stage1_electrical_track_audit.json files found')
for path in files:
    with open(path) as f:
        d = json.load(f)
    ...
    if bool(d.get('parallel_lane_merge_allowed', True)):
        raise SystemExit(f'parallel-lane merge unexpectedly enabled: {path}')
    if bool(d.get('line_to_line_bridge_near_pole_allowed', True)):
        raise SystemExit(f'near-pole line-line bridge unexpectedly enabled: {path}')

    profile = d.get('profile', {})
    sel = d.get('selected_fragment_bridges_csv', '')
    if sel:
        sp = sel.replace('/outputs/', '/review/__INVALID__/') if False else sel
        # Audit paths in JSON are container paths from the original run. Rebase to /review.
        marker = '/stage2/'
        if marker in sel:
            rel = sel.split(marker, 1)[1]
            # rel starts with SID/...; /review is already that SID root, so strip first path element.
            parts = rel.split('/', 1)
            local = '/review/' + (parts[1] if len(parts) == 2 else parts[0])
        else:
            local = ''
        if local and os.path.isfile(local):
            try:
                bf = pd.read_csv(local)
            except pd.errors.EmptyDataError:
                bf = pd.DataFrame()
            if not bf.empty:
                if 'selected' in bf.columns:
                    bf = bf[bf['selected'].astype(str).str.lower().isin(['true','1']) | bf['selected'].eq(True)]
                for r in bf.itertuples(index=False):
                    if bool(getattr(r, 'near_pole_bridge_guard', False)):
                        raise SystemExit(f'selected bridge crosses pole guard: {local}')
                    if float(getattr(r, 'longitudinal_overlap_ft', 0.0)) > float(profile.get('max_longitudinal_overlap_ft', 0.0)) + 1e-9:
                        raise SystemExit(f'selected bridge longitudinal overlap violation: {local}')
                    if float(getattr(r, 'lane_center_offset_ft', 0.0)) > float(profile.get('max_lane_offset_ft', 0.0)) + 1e-9:
                        raise SystemExit(f'selected bridge lane offset violation: {local}')
                    if float(getattr(r, 'endpoint_lateral_jump_ft', 0.0)) > float(profile.get('max_lane_offset_ft', 0.0)) + 1e-9:
                        raise SystemExit(f'selected bridge endpoint lateral violation: {local}')
                    if float(getattr(r, 'axis_angle_deg', 0.0)) > float(profile.get('max_axis_angle_deg', 0.0)) + 1e-9:
                        raise SystemExit(f'selected bridge axis-angle violation: {local}')

    att = d.get('pole_attachments_csv', '')
    if att and marker in att:
        rel = att.split(marker, 1)[1]
        parts = rel.split('/', 1)
        local_att = '/review/' + (parts[1] if len(parts) == 2 else parts[0])
        if os.path.isfile(local_att):
            try:
                af = pd.read_csv(local_att)
            except pd.errors.EmptyDataError:
                af = pd.DataFrame()
            if not af.empty and {'pole_component_id','component_id','anchor_x','anchor_y','anchor_z'} <= set(af.columns):
                seen = {}
                for r in af.itertuples(index=False):
                    key=(str(r.pole_component_id), round(float(r.anchor_x),6), round(float(r.anchor_y),6), round(float(r.anchor_z),6))
                    prev=seen.get(key)
                    if prev is not None and prev != str(r.component_id):
                        raise SystemExit(f'different conductors share identical pole attachment point: {local_att}: {prev} vs {r.component_id}')
                    seen[key]=str(r.component_id)
print(f'STAGE1_ELECTRICAL_SESSION_AUDIT_OK slices={len(files)}')
PY
...
SUMMARY_COUNT=$(find "$STAGE2_HOST" -mindepth 2 -maxdepth 2 -type f -name STAGE2_STAGE1_ELECTRICAL_TRACK_SUMMARY.json | wc -l | tr -d ' ')
...
ARCHIVE="$PACKAGE_HOST/v4_stage2_stage1_electrical_${RUN_ID}.tar.gz"
...
printf '%s\n' "$ARCHIVE" > "$HOME/LATEST_V4_STAGE2_STAGE1_ELECTRICAL_V10_ARCHIVE.txt"
...
echo "============================================================"
echo "V4 STAGE2 STAGE1-ELECTRICAL CANONICAL RUN COMPLETE"
echo "============================================================"
