...
nohup env EXP_REPO="$EXP_REPO" RUN_STAMP="$STAMP" VARIANT_NAME="$VARIANT_NAME" \
  ...
  SCORE_ATOL="${SCORE_ATOL:-1e-4}" COMPILE_MODEL="${COMPILE_MODEL:-1}" \
  COMPILE_MODE="${COMPILE_MODE:-reduce-overhead}" BATCH_SIZE="${BATCH_SIZE:-12}" \
  CHANNELS_LAST="${CHANNELS_LAST:-1}" PINNED_D2H="${PINNED_D2H:-1}" \
  PRUNE_EMBEDDING_HEAD="${PRUNE_EMBEDDING_HEAD:-1}" WARMUP_ITERATIONS="${WARMUP_ITERATIONS:-3}" \
