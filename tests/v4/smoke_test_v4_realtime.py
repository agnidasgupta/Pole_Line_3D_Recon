...
def stage3_contract_test():
    from poleline._compat import implementation_path
    s=implementation_path(Path(__file__).with_name('reconstruct_v4_stage3.py')).read_text()
...
def stage12_world_guard():
    ...
    from poleline._compat import implementation_path
    src=implementation_path(Path(__file__).with_name('v4_stage2_runtime.py')).read_text()
