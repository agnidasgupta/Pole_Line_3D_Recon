#!/usr/bin/env bash
POLELINE_LEGACY_SOURCE="$(cd "$(dirname "${BASH_SOURCE[0]}")/../.." && pwd)/v4/run_v4_stage1_training_on_nebius.sh"
set -euo pipefail
...
nohup docker run --rm \
  --gpus all \
  --mount "type=bind,source=$REPO/v4,target=/workspace/v4,readonly" --mount "type=bind,source=$REPO/v4/..,target=/workspace/poleline_repo,readonly" \
