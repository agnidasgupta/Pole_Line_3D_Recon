...
def _sibling_support(
    ...
    """Find a production-accepted parallel sibling lane for a residual candidate.

    Pole pairs are intentionally not considered. Support comes only from another
    production-accepted conductor in the same slice plus actual Stage1-labelled
    occupied voxels in the candidate.
    """
    ...
    c_span = max(float(np.ptp(cs)), 1e-9)
    c_center = np.median(cpoints, axis=0)
    ...
    for component_id, points in baseline_points.items():
        ...
        # Cross-section distance from candidate center to the baseline lane,
        # combining horizontal perpendicular and vertical offsets.
        _, transverse, vertical = bmodel.project(c_center.reshape(1, 3))
        offset_ft = float(
            math.hypot(float(transverse[0]), float(vertical[0]))
            * float(voxel_size_ft)
        )
        ...
        # Compare both lanes along the candidate axis. This remains stable for
        # nearly parallel lanes and avoids accepting a short crossing branch.
        bs = (baseline["points"][:, :2] - cmodel.center_xy) @ cmodel.axis_xy
        overlap = max(
            0.0,
            min(float(np.max(cs)), float(np.max(bs)))
            - max(float(np.min(cs)), float(np.min(bs))),
        )
        overlap_fraction = float(overlap / c_span)
        if overlap_fraction < float(profile.sibling_min_longitudinal_overlap_fraction):
            continue

        record = {
            "supported": True,
            "sibling_component_id": str(component_id),
            "sibling_axis_angle_deg": float(angle),
            "sibling_cross_section_offset_ft": float(offset_ft),
            "sibling_longitudinal_overlap_fraction": float(overlap_fraction),
        }
        if best is None or (
            record["sibling_longitudinal_overlap_fraction"],
            -record["sibling_axis_angle_deg"],
        ) > (
            best["sibling_longitudinal_overlap_fraction"],
            -best["sibling_axis_angle_deg"],
        ):
            best = record

    if best is None:
        return {"supported": False, "reason": "no_parallel_production_sibling"}
    return best
...
class Stage1ResidualUnionStage2Processor:
    ...
    def process(
            ...
            for index, row in candidate_components.iterrows():
                ...
                if final_accept:
                    candidate_components.at[index, "accept_mode"] = (
                        "stage1_residual_union_" + str(row.get("accept_mode", "accepted"))
                    )
        ...
        audit = {
            **detail["audit"],
            "processor": "production_baseline_plus_stage1_residual_union_v6",
            "production_accepted_line_components": int(len(base_line_ids)),
