...
# Stage2-only bundle-consensus experiment. The output contract is intentionally identical to the
...
EXP_REPO="${EXP_REPO:-/workspace/voxel_poleline/Pole_Line_3D_Recon_v4_stage2_bundle_consensus_v7}"
TOOL_DIR="$EXP_REPO/ops/v4_stage2_bundle_consensus_v7"
...
for file in \
  ...
  self_test_bundle_consensus.py; do
...
SELECTOR_HOST="$TOOL_DIR/select_v4_stage2_bundle_consensus_profile.py"
grep -Fq "BUNDLE_CONSENSUS_SELECTOR_VERSION = \"$SELECTOR_VERSION_EXPECTED\"" "$SELECTOR_HOST" \
...
docker run --rm -i \
  ...
  bash -lc "
    ...
    python -m py_compile \
      /workspace/quality/v4_stage2_bundle_consensus.py \
      /workspace/quality/select_v4_stage2_bundle_consensus_profile.py \
      /workspace/quality/run_v4_stage2_bundle_consensus.py \
      /workspace/quality/self_test_bundle_consensus.py
    python /workspace/quality/self_test_bundle_consensus.py
    python /workspace/quality/select_v4_stage2_bundle_consensus_profile.py --self_test
...
if [ "$PREFLIGHT_ONLY" = "1" ]; then
  echo "============================================================"
  echo "V4 STAGE2 BUNDLE-CONSENSUS V7 PREFLIGHT OK"
  echo "============================================================"
...
printf '%s\n' "$RUN_HOST" > "$HOME/LATEST_V4_STAGE2_BUNDLE_CONSENSUS_V7_RUN.txt"
...
docker run --rm \
  ...
  "$IMAGE" \
  python /workspace/quality/select_v4_stage2_bundle_consensus_profile.py \
...
if [ "$SELECT_EXIT" -ne 0 ]; then
  if [ "$SELECT_EXIT" -eq 3 ] && [ -f "$SELECTION_HOST/NO_SAFE_BUNDLE_CONSENSUS_PROFILE.txt" ]; then
    cp "$SELECTION_HOST/NO_SAFE_BUNDLE_CONSENSUS_PROFILE.txt" "$RUN_HOST/NO_SAFE_BUNDLE_CONSENSUS_PROFILE.txt"
    ...
    ARCHIVE="$PACKAGE_HOST/v4_stage2_bundle_consensus_selection_full_run_v2_${RUN_STAMP}.tar.gz"
    tar -czf "$ARCHIVE" -C "$RUN_HOST" selection logs RUN_INFO.txt NO_SAFE_BUNDLE_CONSENSUS_PROFILE.txt FILE_INVENTORY.txt
    tar -tzf "$ARCHIVE" >/dev/null
    sha256sum "$ARCHIVE" > "${ARCHIVE}.sha256"
    printf '%s\n' "$ARCHIVE" > "$HOME/LATEST_V4_STAGE2_BUNDLE_CONSENSUS_V7_ARCHIVE.txt"
    echo "NO_SAFE_BUNDLE_CONSENSUS_PROFILE"
...
grep -Fq 'SELECTED_SAFE_BUNDLE_CONSENSUS_PROFILE' "$SELECTION_HOST/selection_report.txt" \
...
for manifest in "${MANIFESTS[@]}"; do
  ...
  docker run --rm \
    ...
    -e MPLCONFIGDIR=/tmp/matplotlib \
    "$IMAGE" \
    python /workspace/quality/run_v4_stage2_bundle_consensus.py \
  ...
  [ -f "$S2_SESSION_HOST/STAGE2_BUNDLE_CONSENSUS_SUMMARY.json" ] || fail "bundle-consensus summary missing for $GID"
...
ARCHIVE="$PACKAGE_HOST/v4_stage2_bundle_consensus_${RUN_ID}.tar.gz"
...
echo "============================================================"
echo "V4 STAGE2 BUNDLE-CONSENSUS CANONICAL RUN COMPLETE"
echo "============================================================"
