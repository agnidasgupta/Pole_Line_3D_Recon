#!/usr/bin/env python3
"""Select a production-preserving Stage1 residual-union V4 Stage2 profile with offline GT.
...
from v4_stage2_residual_union import (
    Stage1LabelProfile,
    Stage1ResidualUnionStage2Processor,
...
RESIDUAL_UNION_SELECTOR_VERSION = "residual-union-v6-20260903"
...
def write_profile_env(path: Path, profile: Stage1LabelProfile) -> None:
    with path.open("w") as handle:
        handle.write("SELECTION_STATUS=SELECTED_SAFE_RESIDUAL_UNION_PROFILE\n")
...
def self_test() -> None:
    ...
    print(f"V4_STAGE2_RESIDUAL_UNION_SELECTOR_SELF_TEST_OK version={RESIDUAL_UNION_SELECTOR_VERSION}")


def main() -> None:
    args = cli()
    print(f"RESIDUAL_UNION_SELECTOR_VERSION={RESIDUAL_UNION_SELECTOR_VERSION}", flush=True)
    if args.self_test:
        self_test()
        return
    ...
    (out / "SELECTOR_VERSION.txt").write_text(RESIDUAL_UNION_SELECTOR_VERSION + "\n")
    ...
    for index, profile in enumerate(profiles, 1):
        processor = Stage1ResidualUnionStage2Processor(bundle, calibration_path, profile, grid, args.voxel_size_ft)
    ...
    if selected is None:
        (out / "NO_SAFE_RESIDUAL_UNION_PROFILE.txt").write_text(
            "NO_SAFE_RESIDUAL_UNION_PROFILE\n"
        ...
        print("NO_SAFE_RESIDUAL_UNION_PROFILE")
        raise SystemExit(3)
    ...
    selected_processor = Stage1ResidualUnionStage2Processor(
    ...
    report = {
        "status": "SELECTED_SAFE_RESIDUAL_UNION_PROFILE",
    ...
    with (out / "selection_report.txt").open("w") as handle:
        handle.write("SELECTED_SAFE_RESIDUAL_UNION_PROFILE\n")
    ...
    print("SELECTED_SAFE_RESIDUAL_UNION_PROFILE")
