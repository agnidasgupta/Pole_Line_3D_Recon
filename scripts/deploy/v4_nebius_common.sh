#!/usr/bin/env bash
POLELINE_LEGACY_SOURCE="$(cd "$(dirname "${BASH_SOURCE[0]}")/../.." && pwd)/v4/v4_nebius_common.sh"
...
V4_HERE=$(cd "$(dirname "${POLELINE_LEGACY_SOURCE}")" && pwd)
...
V4_CONTAINER_PREFIX=(
  ...
  --mount "type=bind,source=$V4_HERE,target=/workspace/v4,readonly" --mount "type=bind,source=$V4_HERE/..,target=/workspace/poleline_repo,readonly"
...
v4_backup_code() {
  ...
  tar -C "$V4_REPO_ROOT" --exclude='.git' --exclude='.venv' --exclude='artifacts' --exclude='__pycache__' --exclude='*.pyc' -czf "$backup" .
...
v4_detached_run() {
  ...
  id=$("${V4_DOCKER[@]}" run -d --name "$name" --gpus all \
    ...
    -e TORCH_HOME=/tmp/torch-home -e TORCHINDUCTOR_CACHE_DIR=/tmp/torchinductor -e TRITON_CACHE_DIR=/tmp/triton \
    -e CUDA_CACHE_PATH=/tmp/cuda-cache -e NUMBA_CACHE_DIR=/tmp/numba-cache -e JOBLIB_TEMP_FOLDER=/tmp/joblib -e TMPDIR=/tmp \
    --mount "type=bind,source=$V4_HERE,target=/workspace/v4,readonly" --mount "type=bind,source=$V4_HERE/..,target=/workspace/poleline_repo,readonly" --mount "type=bind,source=$V4_HOST_OUTPUTS,target=/outputs" --mount "type=bind,source=$V4_HOST_INPUT,target=/data/voxel_csv_combined,readonly" \
