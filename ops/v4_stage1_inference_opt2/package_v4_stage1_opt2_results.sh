...
[ -s "$RUN_ROOT/STAGE1_OPT2_POSITIVE_SAFE_COMPLETE.txt" ] || { echo "ERROR: completion marker missing" >&2; exit 1; }
...
[ "$accepted" -eq 30 ] && [ "$failed" -eq 0 ] && [ "$guarded" -eq 30 ] || {
  echo "ERROR: not ready: accepted=$accepted failed=$failed positive_guarded=$guarded" >&2
  exit 1
}
stamp=$(date -u +%Y%m%dT%H%M%SZ)
archive=/home/agni/v4_stage1_opt2_positive_safe_results_${stamp}.tar.gz
