#!/usr/bin/env python3
"""Run V4 Stage 2 from saved Stage-1 artifacts using production-preserving Stage-1 residual union.
...
from v4_stage2_residual_union import Stage1ResidualUnionStage2Processor, profile_from_dict
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description="Stage2-only V4 residual-union experiment preserving production output"
    )
...
def main() -> None:
    ...
    for index, row in enumerate(manifest.itertuples(index=False), 1):
        ...
        if (
            ...
            print(f"[stage2-residual-union] {index}/{len(manifest)} reuse seq={seq}", flush=True)
        ...
        print(
            f"[stage2-residual-union] {index}/{len(manifest)} seq={seq} "
    ...
    atomic_json(summary, output_root / "STAGE2_RESIDUAL_UNION_SUMMARY.json")
    print("V4_STAGE2_RESIDUAL_UNION_SESSION_OK", flush=True)
