#!/usr/bin/env python3
"""Run Stage-2-only exact Stage-1 inferred-line passthrough from saved Stage1."""
...
from v4_stage2_stage1_exact import STAGE1_EXACT_RUNTIME_VERSION, Stage1ExactStage2Processor
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Stage2 exact passthrough of deployed Stage1 line labels")
...
def diagnostic_paths(pole_csv: Path) -> dict[str, Path]:
    ...
    return {
        ...
        "audit": pole_csv.with_name(f"{stem}_stage1_exact_audit.json"),
        ...
        "isolated_voxels": pole_csv.with_name(f"{stem}_isolated_stage1_line_voxels.csv"),
        "edges": pole_csv.with_name(f"{stem}_exact_line_edges.csv"),
    }
...
def main() -> None:
    ...
    for path in (bundle, calibration):
        if not path.is_file():
    ...
    processor = Stage1ExactStage2Processor(
        str(bundle), str(calibration), tuple(args.grid_size), args.voxel_size_ft
    )
    ...
    for index, row in enumerate(manifest.itertuples(index=False), 1):
        ...
        if args.resume and (args.session_filter, seq) in done and pole_csv.is_file() and line_csv.is_file() and vertex_csv.is_file() and ready:
            print(f"[stage2-stage1-exact] {index}/{len(manifest)} reuse seq={seq}", flush=True)
        ...
        isolated = np.asarray(result["isolated_indices"], dtype=np.int64)
        ...
        if args.write_voxel_audit:
            ...
            # Exact passthrough: accepted voxel audit is deliberately identical.
            atomic_csv(voxel_frame, diag["accepted_voxels"], columns=list(voxel_frame.columns))
            isolated_frame = pd.DataFrame({
                "x": coords[isolated, 0] if len(isolated) else np.empty(0, dtype=int),
                "y": coords[isolated, 1] if len(isolated) else np.empty(0, dtype=int),
                "z": coords[isolated, 2] if len(isolated) else np.empty(0, dtype=int),
            })
            atomic_csv(isolated_frame, diag["isolated_voxels"], columns=["x", "y", "z"])
            edge_frame = pd.DataFrame(result["exact_edge_rows"])
            atomic_csv(edge_frame, diag["edges"], columns=[
                "component_id", "x1", "y1", "z1", "x2", "y2", "z2", "edge_step_ft", "line_score_mean"
            ])

        audit = {
            **result["stage1_exact_audit"],
            ...
            "exact_line_edges_csv": str(diag["edges"]) if args.write_voxel_audit else "",
        }
        ...
        upsert_timing(timing_path, {
            ...
            "stage1_exact_line_ms": float(result["timing"]["stage1_exact_line_ms"]),
            ...
            "exact_line_edges": int(audit["exact_line_edges"]),
            "isolated_stage1_line_voxels": int(audit["isolated_stage1_line_voxels"]),
            "max_exact_edge_step_ft": float(audit["max_exact_edge_step_ft"]),
        ...
        print(
            f"[stage2-stage1-exact] {index}/{len(manifest)} seq={seq} "
            f"poles={len(result['poles'])} edges={len(result['lines'])} "
            f"stage1_line={audit['stage1_inferred_line_voxels']} "
            f"preservation={audit['stage1_to_stage2_voxel_preservation']:.4f} "
            f"isolated={audit['isolated_stage1_line_voxels']} stage2={stage2_ms:.1f}ms",
            flush=True,
        )
    ...
    summary = {
        ...
        "stage": 2,
        "completed": True,
        "processor": STAGE1_EXACT_RUNTIME_VERSION,
        "group_id": args.session_filter,
        ...
        "totals": {
            ...
            "exact_line_edges": int(sum(int(a.get("exact_line_edges", 0)) for a in session_audits)),
            "isolated_stage1_line_voxels": int(sum(int(a.get("isolated_stage1_line_voxels", 0)) for a in session_audits)),
        },
    }
    if summary["totals"]["stage1_inferred_line_voxels"] != summary["totals"]["accepted_stage1_line_voxels"]:
        raise RuntimeError("exact passthrough preservation invariant failed")
    atomic_json(summary, output_root / "STAGE2_COMPLETED.json")
    atomic_json(summary, output_root / "STAGE2_STAGE1_EXACT_SUMMARY.json")
    print("V4_STAGE2_STAGE1_EXACT_SESSION_OK", flush=True)
