#!/usr/bin/env python3
"""Render E10 raw timing data as numeric summary plus an accept/reject decision."""
...
def main() -> None:
    ...
    parser.add_argument("--minimum-save-percent", type=float, default=2.0)
    ...
    for frame in (c1, c2, cand):
        if frame.empty:
            raise RuntimeError("E10 timing input is empty")
    ...
    if c1.duplicated(keys).any() or c2.duplicated(keys).any() or cand.duplicated(keys).any():
        raise RuntimeError("E10 duplicate timing slice keys; timing result is NOT_ACCEPTED")
    ...
    required_control_timings = ("csv_read_ms", "sparse_item_prep_ms", "stage1_wall_ms",
                                "stage1_artifact_write_ms", "stage1_manifest_write_ms",
                                "stage2_disk_slice_wall_ms")
    required_candidate_timings = ("csv_read_ms", "sparse_item_prep_ms",
                                  "stage1_inference_ms", "stage1_manifest_write_ms",
                                  "stage2_callback_and_output_ms", "combined_stage1_stage2_ms")
    timing_complete = all(name in control and pd.to_numeric(control[name], errors="coerce").notna().all()
                          for name in required_control_timings) and all(
        name in cand and pd.to_numeric(cand[name], errors="coerce").notna().all()
        for name in required_candidate_timings)
    ...
    per_session = control[["group_id", "combined_control_ms"]].groupby("group_id").mean().join(
        cand[["group_id", "combined_stage1_stage2_ms"]].groupby("group_id").mean(), how="inner"
    )
    faster_sessions = int((per_session["combined_stage1_stage2_ms"] < per_session["combined_control_ms"]).sum())
    p95_ok = float(candidate_summary["p95_ms"]) <= 1.05 * float(control_summary["p95_ms"])
    accepted = (
        ...
        and timing_complete and exact_count == exact_required and saved_percent >= args.minimum_save_percent
        and faster_sessions >= 16 and p95_ok
    )
    raw = {
        "update_date_utc": pd.Timestamp.now("UTC").strftime("%Y-%m-%d"),
        "decision": "ACCEPTED" if accepted else "NOT_ACCEPTED",
        "acceptance_rules": {"expected_sessions": args.expected_sessions, "expected_slices": args.expected_slices, "expected_comparisons": args.expected_comparisons, "stage1_exact_rows": exact_required, "complete_slice_coverage": True, "stage2_output_comparisons_pass": True, "minimum_saved_percent": args.minimum_save_percent, "minimum_faster_sessions": 16, "max_p95_increase_percent": 5.0},
        "observed": {"sessions": session_count, "control_slices": len(c1), "candidate_slices": candidate_all_count, "complete_slice_coverage": complete_coverage, "stage2_comparisons": len(reports), "stage1_exact_rows": exact_count, "stage2_output_comparisons_pass": comparison_pass, "timing_complete": timing_complete, "saved_percent": saved_percent, "faster_sessions": faster_sessions, "p95_non_regression": p95_ok},
        ...
        "candidate_breakdown": {name: describe(cand, name) for name in ["csv_read_ms", "sparse_item_prep_ms", "stage1_inference_ms", "stage1_manifest_write_ms", "stage2_reconstruction_ms", "stage2_write_and_orchestration_ms", "stage2_callback_and_output_ms", "combined_stage1_stage2_ms", "validation_ms_excluded"]},
    ...
    lines = ["# E10 Stage1-to-Stage2 in-memory handoff result", "", f"**Update date: {raw['update_date_utc']} UTC**", f"**Decision: {raw['decision']}**", "", "## Numeric summary", "", "| Metric | Disk control | In-memory candidate |", "| --- | ---: | ---: |", f"| Sessions | {session_count} | {cand['group_id'].nunique()} |", f"| Timed slices (first slice/session excluded) | {control_summary['count']} | {candidate_summary['count']} |", f"| Combined mean (ms) | {control_summary['mean_ms']:.3f} | {candidate_summary['mean_ms']:.3f} |", f"| Combined P50 (ms) | {control_summary['p50_ms']:.3f} | {candidate_summary['p50_ms']:.3f} |", f"| Combined P95 (ms) | {control_summary['p95_ms']:.3f} | {candidate_summary['p95_ms']:.3f} |", f"| Mean time saved | — | {saved_percent:.3f}% |", f"| Stage1 exact payload rows | — | {exact_count}/{exact_required} |", f"| Stage2 output comparisons | — | {sum(state == 'PASS' for state in report_states)}/{args.expected_comparisons} |", f"| Complete slice coverage | — | {'PASS' if complete_coverage else 'FAIL'} |", f"| Complete timing rows | — | {timing_complete} |", "", f"| Sessions faster | — | {faster_sessions}/{len(per_session)} |", f"| Candidate P95 <= control P95 + 5% | — | {p95_ok} |", "", "The candidate timing excludes the exactness diagnostic NPZ read but includes its Stage1 manifest write. Control and candidate both write V10 Stage2 outputs; only the control writes and reloads Stage1 NPZ/JSON. The control Stage2 wall and candidate callback measure analogous but not perfectly identical boundaries. A 2% mean gain, faster majority of sessions, and P95 within 5% are required before acceptance.", "", "## Control timing breakdown", ""]
    ...
    print(f"E10_DECISION={raw['decision']} saved_percent={saved_percent:.3f} stage1_exact={exact_count}/{exact_required} stage2_exact={comparison_pass}")
