#!/usr/bin/env bash
POLELINE_LEGACY_SOURCE="$(cd "$(dirname "${BASH_SOURCE[0]}")/../.." && pwd)/v4/build_v4_realtime_image_on_nebius.sh"
...
if [[ "${V4_BUILD_FOREGROUND:-0}" != 1 ]]; then
  ...
  nohup env V4_BUILD_FOREGROUND=1 IMAGE="$IMAGE" "$POLELINE_LEGACY_SOURCE" >"$LOG" 2>&1 </dev/null &
  PID=$!
