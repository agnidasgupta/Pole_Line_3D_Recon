...
RUN_ROOT="$HOST_OUTPUTS/poleline_voxel_run_session_groups/v4_production/stage1_opt_experiments/$RUN_STAMP"
RUN_C="/outputs/poleline_voxel_run_session_groups/v4_production/stage1_opt_experiments/$RUN_STAMP"
DRIVER_LOG="$RUN_ROOT/STAGE1_OPT_DRIVER.log"
...
printf '%s\n' "$DRIVER_LOG" > /home/agni/LATEST_V4_STAGE1_OPT_DRIVER_LOG.txt
printf '%s\n' "$BASELINE_RUN" > /home/agni/LATEST_V4_STAGE1_OPT_BASELINE.txt

exec > >(tee -a "$DRIVER_LOG") 2>&1
echo "============================================================"
echo "V4 STAGE1 QUALITY-PRESERVING PERFORMANCE EXPERIMENT"
echo "============================================================"
...
docker run --rm --gpus all \
  ...
  -e PYTHONPYCACHEPREFIX=/tmp/pycache "$IMAGE" bash -lc '
    ...
    python /workspace/opt/self_test_v4_stage1_opt.py
...
experiment=v4_stage1_active_gpu_quality_preserving_opt1
repository=$EXP_REPO
...
for manifest in "${MANIFESTS[@]}"; do
  ...
  report_c="$RUN_C/status/$sid.quality_equivalence.json"
  ...
  report="$RUN_ROOT/status/$sid.quality_equivalence.json"
  ...
  timeout --signal=TERM --kill-after=120 "$SESSION_TIMEOUT_SECONDS" docker run --rm --gpus all \
    ...
    python /workspace/opt/run_v4_stage1_opt.py \
      ...
      --progress_json "$progress_c" --batch_size 12 --amp bf16 \
      --compile_model 0 --evaluate_all_cores 0 --gpu_coord_channels 1 \
  ...
  if [ "$rc" -eq 0 ]; then
    ...
    docker run --rm \
      ...
      --mount "type=bind,source=$HOST_OUTPUTS,target=/outputs" \
      --workdir /workspace/v4 -e PYTHONPATH=/workspace/v4:/workspace/opt \
      "$IMAGE" python /workspace/opt/compare_v4_stage1_quality.py \
...
docker run --rm \
  --mount "type=bind,source=$TOOL_DIR,target=/workspace/opt,readonly" \
  --mount "type=bind,source=$HOST_OUTPUTS,target=/outputs" \
  "$IMAGE" python /workspace/opt/summarize_v4_stage1_opt_timing.py \
...
echo "accepted=$accepted failed=$failed equivalent=$equivalent expected=$EXPECTED_SESSIONS"
if [ "$accepted" -eq "$EXPECTED_SESSIONS" ] && [ "$failed" -eq 0 ] && [ "$equivalent" -eq "$EXPECTED_SESSIONS" ]; then
  printf 'completed_utc=%s\naccepted=%s\nfailed=0\nquality_equivalent_sessions=%s\n' \
    "$(date -u +%Y-%m-%dT%H:%M:%SZ)" "$accepted" "$equivalent" \
    > "$RUN_ROOT/PHASE1_STAGE1_OK.txt"
  cp "$RUN_ROOT/PHASE1_STAGE1_OK.txt" "$RUN_ROOT/STAGE1_OPT_EQUIVALENT_COMPLETE.txt"
  find "$RUN_ROOT" -type f ! -name FILE_INVENTORY.txt -printf '%P\t%s\n' | sort > "$RUN_ROOT/FILE_INVENTORY.txt"
  echo "V4_STAGE1_OPT1_ALL_SESSIONS_EQUIVALENT_AND_OK"
  exit 0
fi
echo "V4_STAGE1_OPT1_INCOMPLETE"
exit 1
